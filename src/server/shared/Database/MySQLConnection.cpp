...
bool MySQLConnection::Open()
{
    ...
    if (!mysqlInit)
    {
        sLog->outError("Не удалось инициализировать соединение Mysql с базой данных `%s`", m_connectionInfo.database.c_str());
        return false;
    }
    ...
    if (m_Mysql)
    {
        ...
        sLog->outDetail("Соединение с MySQL датабазой через хост %s", m_connectionInfo.host.c_str());
    ...
    {
        sLog->outError("Не удалось подключиться к базе данных MySQL %s: %s\n", m_connectionInfo.host.c_str(), mysql_error(mysqlInit));
