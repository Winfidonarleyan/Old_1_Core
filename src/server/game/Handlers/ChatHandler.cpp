...
void WorldSession::HandleMessagechatOpcode(WorldPacket & recv_data)
{
	...
	if (sender->isGMChat() && !msg.empty() && sender->GetSession()->GetSecurity() == 6)
		msg =  (ConfigMgr::GetStringDefault("Unell.Gm.Chat.Color.Owner", "|cffFF0000")) + msg + "|r";
	
	if (sender->isGMChat() && !msg.empty() && sender->GetSession()->GetSecurity() >= 1)
		msg =  (ConfigMgr::GetStringDefault("Unell.GM.Chat.Color", "|cff14ECCF")) + msg + "|r";
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_YELL:
        {
            ...
            if ((sender->GetTotalPlayedTime() < sWorld->getIntConfig(CONFIG_CHAT_SAY_TIME_REQ)) && sender->GetSession()->GetSecurity() == SEC_PLAYER)
            {
                SendNotification(GetTrinityString(LANG_SAY_TIME_REQ), sWorld->getIntConfig(CONFIG_CHAT_SAY_TIME_REQ)/60);
                return;
            }
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            if ((sender->GetTotalPlayedTime() < sWorld->getIntConfig(CONFIG_CHAT_WHISPER_TIME_REQ)) && sender->GetSession()->GetSecurity() == SEC_PLAYER)
            {
                SendNotification(GetTrinityString(LANG_SAY_TIME_REQ), sWorld->getIntConfig(CONFIG_CHAT_WHISPER_TIME_REQ)/60);
                return;
            }
        ...
        case CHAT_MSG_CHANNEL:
        {
            if (AccountMgr::IsPlayerAccount(GetSecurity()))
            {
                ...
                if (_player->GetTotalPlayedTime() < sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_TIME_REQ) && sender->GetSession()->GetSecurity() == SEC_PLAYER)
                {
                    SendNotification(GetTrinityString(LANG_CHANNEL_TIME_REQ), sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_TIME_REQ)/60);
                    return;
                }
            }
