...
void Player::FitPlayerInTeam(bool action, Battleground* pBattleGround)
{
    ...
    if (pBattleGround && action)
        SendChatMessage("%sВы играете за %s%s на поле боя %s", MSG_COLOR_CYAN, GetTeam() == ALLIANCE ? MSG_COLOR_DARKBLUE"Альянс" : MSG_COLOR_RED"Орду", MSG_COLOR_CYAN, pBattleGround->GetName());
}
