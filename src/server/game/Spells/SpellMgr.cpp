...
void SpellMgr::LoadSpellRanks()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 spell rank records. DB table `spell_ranks` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Загружено %u spell rank records in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellRequired()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 spell required records. DB table `spell_required` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Загружено %u spell required records in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellLearnSkills()
{
    ...
    sLog->outString(">> Загружено %u Spell Learn Skills from DBC in %u ms", dbc_count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellLearnSpells()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 spell learn spells. DB table `spell_learn_spell` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Загружено %u spell learn spells + %u found in DBC in %u ms", count, dbc_count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellTargetPositions()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 spell target coordinates. DB table `spell_target_position` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Загружено %u spell teleport coordinates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellGroups()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 spell group definitions. DB table `spell_group` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Загружено %u spell group definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellGroupStackRules()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 spell group stack rules. DB table `spell_group_stack_rules` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Загружено %u spell group stack rules in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellProcEvents()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 spell proc event conditions. DB table `spell_proc_event` is empty.");
        sLog->outString();
        return;
    }
    ...
    if (customProc)
        sLog->outString(">> Загружено %u extra and %u custom spell proc event conditions in %u ms",  count, customProc, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outString(">> Загружено %u extra spell proc event conditions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellProcs()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 spell proc conditions and data. DB table `spell_proc` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Загружено %u spell proc conditions and data in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellBonusess()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 spell bonus data. DB table `spell_bonus_data` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Загружено %u extra spell bonus data in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellThreats()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 aggro generating spells. DB table `spell_threat` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Загружено %u SpellThreatEntries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSkillLineAbilityMap()
{
    ...
    sLog->outString(">> Загружено %u SkillLineAbility MultiMap Data in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellPetAuras()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 spell pet auras. DB table `spell_pet_auras` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Загружено %u spell pet auras in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadEnchantCustomAttr()
{
    ...
    sLog->outString(">> Загружено %u custom enchant attributes in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellEnchantProcData()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 spell enchant proc event conditions. DB table `spell_enchant_proc_data` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Загружено %u enchant proc data definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellLinked()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 linked spells. DB table `spell_linked_spell` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Загружено %u linked spells in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadPetLevelupSpellMap()
{
    ...
    sLog->outString(">> Загружено %u pet levelup and default spells for %u families in %u ms", count, family_count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadPetDefaultSpells()
{
    ...
    sLog->outString(">> Загружено addition spells for %u pet spell data entries in %u ms", countData, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
    ...
    sLog->outString(">> Загружено %u summonable creature templates in %u ms", countCreature, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellAreas()
{
    ...
    if (!result)
    {
        sLog->outString(">> Загружено 0 spell area requirements. DB table `spell_area` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Загружено %u spell area requirements in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadSpellInfoStore()
{
    ...
    sLog->outString(">> Загружено spell custom attributes in %u ms", GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void SpellMgr::LoadDbcDataCorrections()
{
    ...
    for (uint32 i = 0; i < sSpellStore.GetNumRows(); ++i)
    {
        ...
        for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j)
        {
            switch (spellInfo->Effect[j])
            {
				...
				if (ConfigMgr::GetBoolDefault("Unell.Glyph.Enchant", true))	
				{
        ...
        case 51678: //WintergraspSiegeEngine Ram set damage radius to 5 yards
            ...
            spellInfo->EffectRadiusIndex[1] = EFFECT_RADIUS_15_YARDS;
            spellInfo->EffectRadiusIndex[2] = EFFECT_RADIUS_20_YARDS;
            break;
            case 71614: // Ice Lock
            ...
            case 45524:
                ...
                break;
            case 49575:
                spellInfo->EffectMiscValueB[0] = 80;
                break;
        default:
            break;
        }
    ...
    sLog->outString(">> Загружены заклинания за %u мс", GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
