...
{
    if (ConfigMgr::GetBoolDefault("Unell.Damage", true))
	{
		if (!(mode & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
		return;
...
void AuraEffect::HandleAuraDummy(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
	...
	case SPELLFAMILY_HUNTER:
		// Animal Handler
		if (m_spellInfo->SpellIconID == 2234 && target->GetTypeId() == TYPEID_PLAYER)
		if (Pet *pet = target->ToPlayer()->GetPet())
		{
	...
	if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
	{
		...
		case 38772: // Grievous Wound
		{
						uint32 percent =
							...
							caster->CalculateSpellDamage(target, GetSpellInfo(), GetEffIndex() + 1) :
							100;
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
	...
	if (GetAuraType() == SPELL_AURA_OBS_MOD_HEALTH)
	{
		...
		// Fel Armor
		if (GetSpellInfo()->SpellIconID == 2297)
			// Demonic Aegis 
...
void AuraEffect::HandlePeriodicManaLeechAuraTick(Unit* target, Unit* caster) const
{
	...
	target->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_TAKE_DAMAGE, GetSpellInfo() ? GetSpellInfo()->Id : 0);
	...
	if (gain_amount)
	{
		...
		target->AddThreat(caster, float(gain) * 0.5f, GetSpellInfo()->GetSchoolMask(), GetSpellInfo());
	}
