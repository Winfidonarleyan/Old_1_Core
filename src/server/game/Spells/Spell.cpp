...
void Spell::DoAllEffectOnTarget(TargetInfo *target)
{
	...
	bool canEffectTrigger = unitTarget->CanProc() && (m_spellInfo->AttributesEx4 & (SPELL_ATTR4_UNK3) ? m_caster != unitTarget : true);
	...
	else if (m_damage > 0)
	{
		...
		if (m_spellInfo->Effects[0].Effect == SPELL_EFFECT_HEALTH_LEECH && damageInfo.damage)
		{
			float healMultiplier = m_spellInfo->Effects[0].CalcDamageMultiplier(m_originalCaster, this);

			// get max possible damage, don't count overkill for heal
			uint32 healthGain = uint32(-unitTarget->GetHealthGain(-damageInfo.damage) * healMultiplier * 3);
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
	 ...
	 if (m_spellInfo->SpellIconID == 156)    // Holy Shock
            ...
            {
                if (m_caster->GetTypeId() == TYPEID_PLAYER && m_spellInfo->Id == 36554 && m_caster->HasUnitState(UNIT_STATE_ROOT))
                    return SPELL_FAILED_ROOTED;
                break;
            }
