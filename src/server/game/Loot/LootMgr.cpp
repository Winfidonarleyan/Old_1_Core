...
void LootStore::ReportUnusedIds(LootIdSet const& lootIdSet) const
{
    ...
    for (LootIdSet::const_iterator itr = lootIdSet.begin(); itr != lootIdSet.end(); ++itr)
        sLog->outErrorDb("В таблице '%s' энтри %d не %s и не ссылается на лут, получается он бесполезен.", GetName(), *itr, GetEntryName());
}

void LootStore::ReportNotExistedId(uint32 id) const
{
    sLog->outErrorDb("В таблице '%s' энтри %d (%s) Не существует, но используется как идентификатор лута в БД.", GetName(), id, GetEntryName());
}
...
bool LootStoreItem::IsValid(LootStore const& store, uint32 entry) const
{
    ...
    {
        sLog->outErrorDb("В таблице '%s' энтри %d итем %d: группа (%u) должен быть меньше %u - пропуск", store.GetName(), entry, itemid, group, 1 << 7);
        return false;
    }

    if (mincountOrRef == 0)
    {
        sLog->outErrorDb("В таблице '%s' энтри %d итем %d: неправильный mincountOrRef (%d) - пропуск", store.GetName(), entry, itemid, mincountOrRef);
        return false;
    }
    ...
    if (mincountOrRef > 0)                                  // item (quest or non-quest) entry, maybe grouped
    {
        ...
        if (!proto)
        {
            sLog->outErrorDb("В таблице '%s' энтри %d итем %d: итем айди не найден в `item_template` - пропуск", store.GetName(), entry, itemid);
            return false;
        }

        if (chance == 0 && group == 0)                      // Zero chance is allowed for grouped entries only
        {
            sLog->outErrorDb("В таблице '%s' entry %d item %d: equal-chanced grouped entry, but group not defined - skipped", store.GetName(), entry, itemid);
            return false;
        }

        if (chance != 0 && chance < 0.000001f)             // loot with low chance
        {
            sLog->outErrorDb("В таблице '%s' entry %d item %d: low chance (%f) - skipped",
        ...
        {
            sLog->outErrorDb("В таблице '%s' entry %d item %d: max count (%u) less that min count (%i) - skipped", store.GetName(), entry, itemid, int32(maxcount), mincountOrRef);
            return false;
        }
    }
    ...
    {
        if (needs_quest)
            sLog->outErrorDb("В таблице '%s' entry %d item %d: quest chance will be treated as non-quest chance", store.GetName(), entry, itemid);
        else if (chance == 0)                              // no chance for the reference
        {
            sLog->outErrorDb("В таблице '%s' entry %d item %d: zero chance is specified for a reference, skipped", store.GetName(), entry, itemid);
            return false;
        }
    }
...
bool Loot::FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool personal, bool noEmptyError, uint16 lootMode /*= LOOT_MODE_DEFAULT*/)
{
    ...
    {
        if (!noEmptyError)
            sLog->outErrorDb("В таблице '%s' loot id #%u used but it doesn't have records.", store.GetName(), lootId);
        return false;
    }
...
void LootTemplate::LootGroup::Verify(LootStore const& lootstore, uint32 id, uint8 group_id) const
{
    ...
    {
        sLog->outErrorDb("В таблице '%s' entry %u group %d has total chance > 100%% (%f)", lootstore.GetName(), id, group_id, chance);
    }

    if (chance >= 100.0f && !EqualChanced.empty())
    {
        sLog->outErrorDb("В таблице '%s' entry %u group %d has items with chance=0%% but group total chance >= 100%% (%f)", lootstore.GetName(), id, group_id, chance);
    }
}
...
{
    sLog->outString("Загрузка creature loot templates...");
    ...
    if (count)
        sLog->outString(">> Загружено %u лут нпц за %u мс", count, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outErrorDb(">> Загружено 0 creature loot templates. DB table `creature_loot_template` is empty");
...
{
    sLog->outString("Загрузка disenchanting loot templates...");
    ...
    if (count)
        sLog->outString(">> Загружено %u disenchanting loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outErrorDb(">> Загружено 0 disenchanting loot templates. DB table `disenchant_loot_template` is empty");
    sLog->outString();
}

void LoadLootTemplates_Fishing()
{
    sLog->outString("Загрузка fishing loot templates...");
    ...
    if (count)
        sLog->outString(">> Загружено %u fishing loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outErrorDb(">> Загружено 0 fishing loot templates. DB table `fishing_loot_template` is empty");
...
{
    sLog->outString("Загрузка gameobject loot templates...");
    ...
    if (count)
        sLog->outString(">> Загружено %u gameobject loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outErrorDb(">> Загружено 0 gameobject loot templates. DB table `gameobject_loot_template` is empty");
...
{
    sLog->outString("Загрузка item loot templates...");
    ...
    if (count)
        sLog->outString(">> Загружено %u prospecting loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outErrorDb(">> Загружено 0 prospecting loot templates. DB table `item_loot_template` is empty");
...
{
    sLog->outString("Загрузка milling loot templates...");
    ...
    if (count)
        sLog->outString(">> Загружено %u milling loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outErrorDb(">> Загружено 0 milling loot templates. DB table `milling_loot_template` is empty");
...
{
    sLog->outString("Загрузка pickpocketing loot templates...");
    ...
    if (count)
        sLog->outString(">> Загружено %u pickpocketing loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outErrorDb(">> Загружено 0 pickpocketing loot templates. DB table `pickpocketing_loot_template` is empty");
...
{
    sLog->outString("Загрузка prospecting loot templates...");
    ...
    if (count)
        ...
        sLog->outErrorDb(">> Загружено 0 prospecting loot templates. DB table `prospecting_loot_template` is empty");
...
{
    sLog->outString("Загрузка mail loot templates...");
    ...
    if (count)
        sLog->outString(">> Загружено %u mail loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outErrorDb(">> Загружено 0 mail loot templates. DB table `mail_loot_template` is empty");
...
{
    sLog->outString("Загрузка skinning loot templates...");
    ...
    if (count)
        sLog->outString(">> Загружено %u skinning loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outErrorDb(">> Загружено 0 skinning loot templates. DB table `skinning_loot_template` is empty");
...
{
    sLog->outString("Загрузка spell loot templates...");
    ...
    if (count)
        sLog->outString(">> Загружено %u spell loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outErrorDb(">> Загружено 0 spell loot templates. DB table `spell_loot_template` is empty");
    sLog->outString();
}

void LoadLootTemplates_Reference()
{
    sLog->outString("Загрузка reference loot templates...");
    ...
    sLog->outString(">> Загружено refence loot templates in %u ms", GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
