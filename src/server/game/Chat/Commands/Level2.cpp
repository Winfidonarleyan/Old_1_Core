...
bool ChatHandler::HandleMuteCommand(const char* args)
{
    ...
    std::string announce;
    std::string mutereasonstr = "Укажите причину";
    if (mutereason != NULL)
         mutereasonstr = mutereason;
    
    if(!mutereason) 
    { 
        PSendSysMessage("Укажите причину"); 
	...
	/*if (notspeaktime < 0)
		uint32 notspeaktime = 1000000000;*/
    ...
    {
        sWorld->SendWorldText(LANG_UNELL_MUTE_ANNOUNCER, nameLink.c_str(), notspeaktime, nameself.c_str(), mutereasonstr.c_str());
	}
    return true;
}
