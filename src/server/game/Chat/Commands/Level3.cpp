...
bool ChatHandler::HandleAHBotOptionsCommand(const char *args)
{
     ...
     else if (strncmp(opt,"percentages",l) == 0)
     {
         ...
         if ((!ahMapIdStr) || (!param14))
         {
             ...
             PSendSysMessage("The total must add up to 100 persent's");
             return false;
         }
...
bool ChatHandler::HandleHealCommand(const char* args)
{
    ...
    char* healStr = strtok((char*)args, " ");
    if (!healStr)
        return false;

    int32 heal_int = atoi((char*)healStr);
    if (heal_int <= 0)
        return false;

    uint32 heal = heal_int;

    m_session->GetPlayer()->DealHeal(target, heal);
...
bool ChatHandler::HandleBanCharacterCommand(const char *args)
{
	...
	std::string nameself = m_session->GetPlayerName();
   ...
   std::string announce;
    ...
    switch (sWorld->BanCharacter(name, duration, reason, m_session ? m_session->GetPlayerName() : ""))
    {
        case BAN_SUCCESS:
        {
            ...
            {
                // announce = "|cff6C8CD5Персонажу |cff14ECCF";
                // announce += name;
                if (atoi(duration) > 0)
                {
                   // announce += "|cff6C8CD5 был выдан бан на |cff14ECCF";
                   // announce += secsToTimeString(TimeStringToSecs(duration), true).c_str();
				   sWorld->SendWorldText(LANG_UNELL_CHAR_BAN_ANNOUNCER, name.c_str(), secsToTimeString(TimeStringToSecs(duration), false, false).c_str(), nameself.c_str(), reason);
                }
                else
                // announce += "|cff6C8CD5 был выдан перманентный бан.";
                // announce += "|cff6C8CD5 Причина: |cff14ECCF";
                // announce += reason;
                // HandleAnnounceCommand(announce.c_str());
				sWorld->SendWorldText(LANG_UNELL_CHAR_PERNAMENTBAN_ANNOUNCER, name.c_str(), nameself.c_str(), reason);
            }
            break;
        }
        case BAN_NOTFOUND:
            ...
            if(sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
            {
                switch (mode)
                {
                    case BAN_ACCOUNT:
						...
						if (atoi(duration) > 0)												
						{
						sWorld->SendWorldText(LANG_UNELL_ACC_BAN_ANNOUNCER, nameOrIP.c_str(), secsToTimeString(TimeStringToSecs(duration), false, false).c_str(), nameself.c_str(), reason);	
						}
						else
						sWorld->SendWorldText(LANG_UNELL_ACC_PERNAMENTBAN_ANNOUNCER, nameOrIP.c_str(), nameself.c_str(), reason);
                        break;
                    case BAN_CHARACTER:
                        // announce = "Персонажу ";
						if (atoi(duration) > 0)
						{
						sWorld->SendWorldText(LANG_UNELL_CHAR_BAN_ANNOUNCER, nameOrIP.c_str(), secsToTimeString(TimeStringToSecs(duration), false, false).c_str(), nameself.c_str(), reason);	
						}
						else
						sWorld->SendWorldText(LANG_UNELL_CHAR_PERNAMENTBAN_ANNOUNCER, nameOrIP.c_str(), nameself.c_str(), reason);
                        break;
                    case BAN_IP:
                        // announce = "На IP ";
						if (atoi(duration) > 0)
						{
						sWorld->SendWorldText(LANG_UNELL_IP_BAN_ANNOUNCER, nameOrIP.c_str(), secsToTimeString(TimeStringToSecs(duration), false, false).c_str(), nameself.c_str(), reason);	
						}
						else
						sWorld->SendWorldText(LANG_UNELL_IP_PERNAMENTBAN_ANNOUNCER, nameOrIP.c_str(), nameself.c_str(), reason);
                        break;
                }
               ...
               // {
                    // announce += "|cff6C8CD5 был выдан бан на |cff14ECCF";
                    // announce += secsToTimeString(TimeStringToSecs(duration), true).c_str();
                    // announce += "|cff6C8CD5 минут.";
					//sWorld->SendWorldText(LANG_UNELL_BAN_ANNOUNCER, announce, nameOrIP.c_str(), secsToTimeString(TimeStringToSecs(duration), true).c_str(), reason);
              //  }
                // else
                // announce += "|cff6C8CD5 был выдан перманентный бан.";
                // announce += "|cff6C8CD5 Причина: |cff14ECCF";
                // announce += reason;
                // HandleAnnounceCommand(announce.c_str());
				// sWorld->SendWorldText(LANG_UNELL_PERNAMENTBAN_ANNOUNCER, nameOrIP.c_str(), reason);
            }
...
bool ChatHandler::HandleInstanceUnbindCommand(const char *args)
{
    ...
    for (uint8 i = 0; i < MAX_DIFFICULTY; ++i)
    {
        ...
        for (Player::BoundInstancesMap::iterator itr = binds.begin(); itr != binds.end();)
        {
            ...
            {
                std::string timeleft = GetTimeString(save->GetResetTime() - time(NULL));
                PSendSysMessage("Сброс кд в карте: %d подземелье: %d perm: %s diff: %d canReset: %s TTR: %s", itr->first, save->GetInstanceId(), itr->second.perm ? "yes" : "no", save->GetDifficulty(), save->CanReset() ? "yes" : "no", timeleft.c_str());
    ...
    PSendSysMessage("Количество сбросов КД инстов: %d", counter);
    return true;
}
...
bool ChatHandler::HandleUnFreezeCommand(const char *args)
{
    ...
    else
    {
        if (targetName)
        {
			...
			DELETE FROM character_aura WHERE character_aura.spell = 9454 AND character_aura.guid = ?
            stmt->setUInt32(0, lowGuid);
            CharacterDatabase.Execute(stmt);*/
			
			CharacterDatabase.PExecute("DELETE FROM character_aura WHERE character_aura.spell = 9454 AND character_aura.guid = %u", lowGuid);
