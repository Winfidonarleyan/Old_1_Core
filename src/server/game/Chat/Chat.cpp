...
ChatCommand* ChatHandler::getCommandTable()
{
    ...
    if (LoadCommandTable())
    {
        ...
        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_COMMANDS);
...
bool ChatHandler::ShowHelpForSubCommands(ChatCommand* table, char const* cmd, char const* subcmd)
{
    ...
    for (uint32 i = 0; table[i].Name != NULL; ++i)
    {
        // must be available (ignore handler existence for show command with possibe available subcommands
