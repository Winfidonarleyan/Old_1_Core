...
void Player::UpdateBlockPercentage()
{
    ...
    if (CanBlock())
    {
		...
		value = value > sWorld->getFloatConfig(CONFIG_MAX_BLOCK) ? sWorld->getFloatConfig(CONFIG_MAX_BLOCK) : value;
		
    }
    SetStatFloatValue(PLAYER_BLOCK_PERCENTAGE, value);
}

void Player::UpdateCritPercentage(WeaponAttackType attType) // Криты Unell
{
    ...
    // value = value < 0.0f ? 0.0f : value;
	value = value > sWorld->getFloatConfig(CONFIG_MAX_CRIT) ? sWorld->getFloatConfig(CONFIG_MAX_CRIT) : value; // Криты Unell
...
void Player::UpdateParryPercentage()
{
    ...
    if (CanParry() && parry_cap[pclass] > 0.0f)
    {
        ...
        // value = value < 0.0f ? 0.0f : value;
		value = value > sWorld->getFloatConfig(CONFIG_MAX_PARRY) ? sWorld->getFloatConfig(CONFIG_MAX_PARRY) : value;
    }
...
void Player::UpdateDodgePercentage()
{
    ...
    value = value > sWorld->getFloatConfig(CONFIG_MAX_DODGE) ? sWorld->getFloatConfig(CONFIG_MAX_DODGE) : value;
	
    // value = value < 90.000000f ? 90.000000f : value;
...
void Player::UpdateSpellCritChance(uint32 school)
{
	...
	crit = crit > sWorld->getFloatConfig(CONFIG_MAX_CRIT) ? sWorld->getFloatConfig(CONFIG_MAX_CRIT) : crit;
