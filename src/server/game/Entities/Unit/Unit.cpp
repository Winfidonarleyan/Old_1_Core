...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    if(victim && damage >= victim->GetHealth() && this->GetTypeId() == TYPEID_PLAYER)
        ((Player*)this)->UpdateKillingTimedAchievementProgress(victim->GetEntry(), victim->GetTypeId());
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
	...
	if (!victim->isAlive() || victim->HasUnitState(UNIT_STATE_IN_FLIGHT) || (victim->HasUnitState(UNIT_STATE_ONVEHICLE) && victim->GetVehicleBase() != this) || (victim->GetTypeId() == TYPEID_UNIT && (victim->ToCreature()->IsInEvadeMode())))
		return;
...
uint32 Unit::CalcArmorReducedDamage(Unit* victim, const uint32 damage, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/)
{
	...
	if (GetTypeId() == TYPEID_PLAYER)
	{
		...
		float targetLevel = (float)victim->getLevel();
		float armorPen = 400.0f + 85.0f * targetLevel;
		if (targetLevel > 59.0f)
			armorPen += 382.5f * (targetLevel - 59.0f); // 85.0f * 4.5f = 382.5f
		// calculate  Armor Penetration cap
		float armorRed = (armor + armorPen) / 3.0f;
		if (armor < armorRed)
			armorRed = armor;

		float armorPenCoef = CalculatePctF(armorRed, ToPlayer()->GetRatingBonusValue(CR_ARMOR_PENETRATION));

		if (armorPenCoef > 1.0f)
			armorPenCoef = 1.0f;

		armorRed *= armorPenCoef;
		armor -= armorRed;
	}
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (victim)
            {
				if (!spellProto->IsPositive())
                {
                    ...
                    int32 modifer=0;
                    AuraEffectList const& mTotalAuraList = victim->GetAuraEffectsByType(SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE);
                    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
                    {
                        if(!((*i)->GetMiscValue() & schoolMask))
                            continue;
                    ...
                    modifer=0;
                    AuraEffectList const& mTotalAuraList_ = victim->GetAuraEffectsByType(SPELL_AURA_MOD_ATTACKER_SPELL_AND_WEAPON_CRIT_CHANCE);
                    for (AuraEffectList::const_iterator i = mTotalAuraList_.begin(); i != mTotalAuraList_.end(); ++i)
                    {
                        ...
                        if((*i)->GetSpellInfo()->SpellIconID == 237 || (*i)->GetSpellInfo()->Dispel == DISPEL_POISON || (*i)->GetId() == 30708)
                        {
                            if(modifer<(*i)->GetAmount())
                                modifer=(*i)->GetAmount();
                        }
                ...
                for (AuraEffectList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
                {
                    ...
                    switch((*i)->GetMiscValue())
                    {
                        ...
                        case  849: modChance+= 17;
                            if (!victim->isFrozen() && !victim->HasAuraState(AURA_STATE_FROZEN, spellProto, this))
                                break;
...
uint32 Unit::SpellHealingBonus(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    // No bonus for heal by Warlock Healthstones
...
void Unit::ModSpellCastTime(SpellInfo const* spellProto, int32 & castTime, Spell* spell)
{
    ...
    // called from caster
		...
		if (!(spellProto->Attributes & (SPELL_ATTR0_ABILITY | SPELL_ATTR0_TRADESPELL)) && (spellProto->SpellFamilyName || spellProto->Id == 51963))
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    else                                                // creature died
    {
        ...
        if (creature && creature->isWorldBoss())
            sScriptMgr->BossJustDied(creature);
