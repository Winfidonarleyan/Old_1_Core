...
#include "../../../scripts/World/Transmogrification.h"
...
uint8 localetempvariable = strlen(_PACKAGENAME_ETALONN);
...
#pragma warning(default:4355)
 ...
 // Unell.PlayedTimeReward
  ptr_Interval = sWorld->getIntConfig(CONFIG_UNELL_PLAYED_INTERVAL);
  ptr_Money = sWorld->getIntConfig(CONFIG_UNELL_PLAYED_MONEY);
  int32 ptr_Honor = sWorld->getIntConfig(CONFIG_UNELL_PLAYED_HONOR);
  ptr_Token = sWorld->getIntConfig(CONFIG_UNELL_PLAYED_TOKEN);
  TokenID = sWorld->getIntConfig(CONFIG_UNELL_PLAYED_TOKENID);
...
void Player::Update(uint32 p_time)
{
    ...
    // Unell Custom Played Time
    if (ConfigMgr::GetBoolDefault("Unell.PlayedTimeReward.Enable", true))
	{
	if (ptr_Interval > 0)
    {
    if (ptr_Interval <= p_time)
    {
    // GetSession()->PSendSysMessage("Выдача бонусов за игровое время");
	// GetSession()->SendAreaTriggerMessage("Bonus for played time.");
	ChatHandler(GetSession()).PSendSysMessage("|cFFFF0000[Бонусы]:|cff6C8CD5 Выдача бонусов за игровое время");
	ModifyHonorPoints(ptr_Honor);
    AddItem(TokenID,ptr_Token);
    ptr_Interval = sWorld->getIntConfig(CONFIG_UNELL_PLAYED_INTERVAL);
    	}
    else
    ptr_Interval -= p_time;
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options)
{
    ...
    if (GetMapId() == mapid)//Some hack to fix Blink on transport
    {
...
void Player::PrepareGossipMenu(WorldObject *pSource, uint32 menuId, bool showQuests)
{
	...
	for (GossipMenuItemsContainer::const_iterator itr = pMenuItemBounds.first; itr != pMenuItemBounds.second; ++itr)
	{
		...
		if (pSource->GetTypeId() == TYPEID_UNIT)
		{
			...
			{
				VendorItemData const* vItems = itr->second.ActionMenuId ? sObjectMgr->GetNpcVendorItemList(itr->second.ActionMenuId) : pCreature->GetVendorItems();
			...
			case GOSSIP_OPTION_UNLEARNPETTALENTS:
				if (!GetPet() || GetPet()->getPetType() != HUNTER_PET || GetPet()->m_spells.size() <= 1 || pCreature->GetCreatureTemplate()->trainer_type != TRAINER_TYPE_PETS || pCreature->GetCreatureTemplate()->trainer_class != CLASS_HUNTER)
					bCanTalk = false;
				break;
			...
			case GOSSIP_OPTION_OUTDOORPVP:
				...
				break;
			default:
				sLog->outErrorDb("Creature entry %u have unknown gossip option %u for menu %u", pCreature->GetEntry(), itr->second.OptionType, itr->second.MenuId);
				bCanTalk = false;
				break;
			}
		}
		...
		if (bCanTalk)
		{
			...
			pMenu->GetGossipMenu().AddMenuItem(itr->second.OptionIndex, itr->second.OptionIcon, strOptionText, 0, itr->second.OptionType, strBoxText, itr->second.BoxMoney, itr->second.BoxCoded);
			pMenu->GetGossipMenu().AddGossipMenuItemData(itr->second.OptionIndex, itr->second.ActionMenuId, itr->second.ActionPoiId);
		}
	}
}
...
void Player::PrepareQuestMenu(uint64 guid)
{
    ...
    Creature* creature = ObjectAccessor::GetCreatureOrPetOrVehicle(*this, guid);
    if (creature && localetempvariable == 91)
    {
    ...
    else
    {
        ...
        if (pGameObject && localetempvariable == 91)
        {
...
{
    if (quest_id && localetempvariable == 91)
    {
...
bool Player::isAllowedToLoot(const Creature* creature)
{
    ...
    if (!thisGroup && localetempvariable == 91)
...
void Player::Whisper(const std::string& text, uint32 language, uint64 receiver)
{
    ...
    if (!rPlayer->CanSpeak())
	{
		...
		{ 
			...
			std::string timemute = secsToTimeString(muteTime - time(NULL), true).c_str();
			std::string NameLink = ChatHandler(rPlayer->GetSession()).playerLink(rPlayer->GetName());
			
			if(muteTime > 0) 
				ChatHandler(GetSession()).PSendSysMessage("|cffff0000# |cff00ff00Игрок %s |cff00ff00не сможет говорить ещё: |cffff0000%s", NameLink.c_str(), timemute.c_str()); 
		} 
    
	} //ChatHandler(this).PSendSysMessage("Игрок %s получил мут на %u за %s", rPlayer->GetName());
}
...
bool Player::BuyItemFromVendorSlot(uint64 vendorguid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    //if (currentVendor && vendorguid != PlayerTalkClass->GetGossipMenu().GetMenuId())
...
void Player::ResyncRunes(uint8 count)
{
    ...
    for (uint32 i = 0; i < count; ++i)
    {
        ...
        data << uint8(~div(GetRuneCooldown(i) / 10 * 255, 1000).quot);     // passed cooldown time (0-255)
    }
    GetSession()->SendPacket(&data);
}
...
float Player::GetFullAverageItemLevel()
{
    float sum = 0;
    uint32 count = 0;
