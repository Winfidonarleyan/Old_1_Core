...
struct AchievementTimedData
{
    AchievementTimedData(uint32 achievement, uint32 time, uint32 maxprogress)
    {
        Achievement = achievement; TotalTime = time; MaxProgress = maxprogress;
        Progress = 0; TimeNow = 0;
    }

    bool IsCompleted() { return (Progress>=MaxProgress); }
    bool IsFailed() { return (TimeNow>=TotalTime); }
...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        std::list<AchievementTimedData> TimedAchievements;
        AchievementTimedData* GetTimedAchievement(uint32 AchievementId)
        {
            ...
            {
                if((itr)->Achievement == AchievementId)
                {
                    return &(*itr);
                }
            }
        ...
        void SetTimedAchievement(uint32 AchievementId, uint32 time, uint32 maxprogress);
        void UpdateTimedAchievementProgress(uint32 AchievementId, uint32 count)
        {
            if(TimedAchievements.empty())
                return;
            for(std::list<AchievementTimedData>::iterator itr = TimedAchievements.begin(); itr != TimedAchievements.end(); ++itr)
        ...
        void UpdateKillingTimedAchievementProgress(uint32 entry, uint8 type);
