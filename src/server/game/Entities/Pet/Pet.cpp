            ...
            switch (GetEntry())
            {
				...
				case 27829: // Ebon Gargoyle
				{
								...
								if (Player *owner = m_owner->ToPlayer()) // get 100% of owning player's haste
								{
									...
									SetCreateHealth(uint32(owner->GetMaxHealth()*0.4)); // hp must be 0.8x of DK hp
								}
								// Impurity
								...
								SetBonusDamage(int32(m_owner->GetTotalAttackPowerValue(BASE_ATTACK) * 0.6f * impurityMod));
								SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(petlevel - (petlevel / 4)));
								SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(petlevel + (petlevel / 4)));
								break;
				}
