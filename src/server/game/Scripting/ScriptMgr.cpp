            ...
            if (script->IsDatabaseBound())
            {
                ...
                if (id)
                {
                    ...
                    else
                    {
                        ...
                        sLog->outError("Скрипт '%s' Уже назначается с тем же именем скрипта, поэтому скрипт не может работать.",
                ...
                else
                {
                    ...
                    if (script->GetName().find("example") == std::string::npos && script->GetName().find("Smart") == std::string::npos)
                        sLog->outErrorDb("Скрипт '%s' Не имеет имени скрипта, назначенного в базе данных.",
                            script->GetName().c_str());
                }
            }
            else
...
void ScriptMgr::Initialize()
{
    ...
    sLog->outString(">> Загружено %u C++ scripts in %u ms", GetScriptCount(), GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void ScriptMgr::BossJustDied(Creature* creature)
{
    FOREACH_SCRIPT(BossScript)->BossJustDied(creature);
}
...
BossScript::BossScript(const char* name)
    : ScriptObject(name)
{
    ScriptRegistry<BossScript>::AddScript(this);
}
...
template class ScriptRegistry<BossScript>;
