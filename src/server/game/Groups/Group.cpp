...
uint32 Group::IsGuildGroup()
{
    uint32 guildmembercount = 0;
    uint32 groupmembercount = GetMembersCount();
    float koef = 0;
    ...
    for (GroupReference* itr = GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        if (member->GetGuildId() == leader->GetGuildId())
            guildmembercount += 1;
    }

    koef = guildmembercount / float(GetMembersCount());

    if (leader->GetMap()->IsNonRaidDungeon())
        if (koef >= 0.3f && groupmembercount >= 3)
            return leader->GetGuildId();

    if (leader->GetMap()->IsRaid())
    {
        switch (leader->GetMap()->GetDifficulty())
        {
            case RAID_DIFFICULTY_10MAN_NORMAL:
            case RAID_DIFFICULTY_10MAN_HEROIC:
                if (koef >= 0.8f && groupmembercount >= 8)
                    return leader->GetGuildId();
            case RAID_DIFFICULTY_25MAN_NORMAL:
            case RAID_DIFFICULTY_25MAN_HEROIC:
                if (koef >= 0.8f && groupmembercount >= 20)
                    return leader->GetGuildId();
        }
    }
