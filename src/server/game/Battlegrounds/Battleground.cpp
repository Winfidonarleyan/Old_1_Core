...
void Battleground::EndBattleground(uint32 winner)
{
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
		...
		if (isRated())
		{
			...
			{
				player->ModifyHonorPoints(sWorld->getIntConfig(CONFIG_END_ARENA_ADD_WIN));
				player->AddItem(sWorld->getIntConfig(CONFIG_END_ARENA_ADD_WIN), sWorld->getIntConfig(CONFIG_END_ARENA_ADD_ITEM_COUNT));
			}
			else{
				player->ModifyHonorPoints(sWorld->getIntConfig(CONFIG_END_ARENA_ADD_LOST));
			}
		}
	...
	if (team == winner)
		 {
				...
				reward_wsg = 20558, // Варсонг
				reward_ab = 20559, // Низина Арати
				reward_av = 20560, // Альтерак
				reward_IOQ = 47395,
				reward_SOTA = 42425,
				reward_eos = 29024; // Око бури
...
void Battleground::AddPlayer(Player* player)
{
    ...
    BattlegroundQueueTypeId bgQueueTypeId = sBattlegroundMgr->BGQueueTypeId(m_TypeID, GetArenaType());
    uint32 queueSlot = player->GetBattlegroundQueueIndex(bgQueueTypeId);
    sBattlegroundMgr->BuildBattlegroundStatusPacket(&status, this, queueSlot, STATUS_IN_PROGRESS, 0, GetStartTime(), GetArenaType(), player->GetTeam());    player->GetSession()->SendPacket(&status);
