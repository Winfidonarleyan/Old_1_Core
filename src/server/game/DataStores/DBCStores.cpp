...
DBCStorage <ItemDisplayInfoEntry> sItemDisplayInfoStore(ItemDisplayTemplateEntryfmt); // Transmogrification
...
void LoadDBCStores(const std::string& datadbcPath)
{
    uint32 oldMSTime = getMSTime();

    //std::string dbcPath = dataPath+"dbc/";
	std::string dbcPath = datadbcPath;
    ...
    LoadDBC(availableDbcLocales, bad_dbc_files, sItemDisplayInfoStore,        dbcPath, "ItemDisplayInfo.dbc");  // Transmogrification
    ...
    {
        sLog->outError("Incorrect DataDir value in worldserver.conf or ALL required *.dbc files (%d) not found by path: %sdbc", DBCFileCount, datadbcPath.c_str());
        exit(1);
    }
