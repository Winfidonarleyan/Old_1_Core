...
void ConditionMgr::LoadConditions(bool isReload)
{
    ...
    if (!result)
    {
        sLog->outErrorDb(">> Загружено 0 запретов. DB таблица `conditions` не найдена или не могла быть загружена!");
        sLog->outString();
        return;
    }
    ...
    do
    {
        ...
        if (iConditionTypeOrReference < 0)//it has a reference
        {
            ...
            {
                sLog->outErrorDb("Condition reference %i is referencing self, пропуск", iSourceTypeOrReferenceId);
                delete cond;
                continue;
            }
        ...
        {
            sLog->outErrorDb("Запрет типа %u не допоустил значения типа SourceGroup = %u!", uint32(cond->SourceType), cond->SourceGroup);
            delete cond;
            continue;
        }
        if (cond->SourceId && !CanHaveSourceIdSet(cond->SourceType))
        {
            sLog->outErrorDb("Запрет типа %u недопустил значения типа SourceId = %u!", uint32(cond->SourceType), cond->SourceId);
            delete cond;
            continue;
        }
        ...
        if (cond->SourceGroup)
        {
            ...
            if (!valid)
            {
                sLog->outErrorDb("Не обрабатывается сгруппированное условие, SourceGroup %u", cond->SourceGroup);
                delete cond;
            }
            else
            {
    ...
    sLog->outString(">> Загружено %u запретов за %u мс", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
bool ConditionMgr::addToLootTemplate(Condition* cond, LootTemplate* loot)
{
    if (!loot)
    {
        sLog->outErrorDb("ConditionMgr: LootTemplate %u не найдено", cond->SourceGroup);
        return false;
    }
    ...
    sLog->outErrorDb("ConditionMgr: Итем %u не найден в LootTemplate %u", cond->SourceEntry, cond->SourceGroup);
    return false;
}
...
bool ConditionMgr::addToGossipMenus(Condition* cond)
{
    ...
    sLog->outErrorDb("addToGossipMenus: GossipMenu %u не найдено", cond->SourceGroup);
    return false;
}
...
bool ConditionMgr::addToGossipMenuItems(Condition* cond)
{
    ...
    sLog->outErrorDb("addToGossipMenuItems: GossipMenuId %u Итем %u не найден", cond->SourceGroup, cond->SourceEntry);
    return false;
}
    ...
    for (std::list<uint32>::iterator itr = sharedMasks.begin(); itr != sharedMasks.end(); ++itr)
    {
        ...
        if (uint32 commonMask = *itr & conditionEffMask)
        {
            ...
            if (sharedList)
            {
                ...
                {
                    sLog->outErrorDb("SourceEntry %u в таблице `condition`, имеет неправильное значение SourceGroup %u (spell effectMask) - "
                        "Маски эффектов перекрываются (все значения SourceGroup с заданным набором бит должны быть равны) - игнорирование.", cond->SourceEntry, cond->SourceGroup);
                    return false;
                }
            }
...
bool ConditionMgr::isSourceTypeValid(Condition* cond)
{
    ...
    {
        sLog->outErrorDb("Неправильный ConditionSourceType %u в `condition`, игнорирование.", uint32(cond->SourceType));
        return false;
    }
    ...
    switch (cond->SourceType)
    {
        case CONDITION_SOURCE_TYPE_CREATURE_LOOT_TEMPLATE:
        {
            ...
            {
                sLog->outErrorDb("SourceGroup %u в `condition`, не существует в таблице `creature_loot_template`, игнорирование.", cond->SourceGroup);
                return false;
            }
            ...
            {
                sLog->outErrorDb("SourceType %u, SourceEntry %u в `condition`, не существует в таблице `item_template`, игнорирование.", cond->SourceType, cond->SourceEntry);
                return false;
            }
            break;
        }
        case CONDITION_SOURCE_TYPE_DISENCHANT_LOOT_TEMPLATE:
        {
            ...
            {
                sLog->outErrorDb("SourceGroup %u в `condition`, не существует в таблице `disenchant_loot_template`, игнорирование.", cond->SourceGroup);
                return false;
            }
            ...
            if (!pItemProto && !loot->isReference(cond->SourceEntry))
            {
                sLog->outErrorDb("SourceType %u, SourceEntry %u в `condition`, не существует в таблице `item_template`, игнорнирование.", cond->SourceType, cond->SourceEntry);
                return false;
            }
            break;
        }
        case CONDITION_SOURCE_TYPE_FISHING_LOOT_TEMPLATE:
        {
            ...
            {
                sLog->outErrorDb("SourceGroup %u в `condition`, не существует в таблице `fishing_loot_template`, игнорнирование.", cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_GAMEOBJECT_LOOT_TEMPLATE:
        {
            ...
            {
                sLog->outErrorDb("SourceGroup %u в `condition`, не существует в таблице `gameobject_loot_template`, игнорнирование.", cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_ITEM_LOOT_TEMPLATE:
        {
            ...
            {
                sLog->outErrorDb("SourceGroup %u в `condition`, не существует в таблице `item_loot_template`, игнорнирование.", cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_MAIL_LOOT_TEMPLATE:
        {
            ...
            {
                sLog->outErrorDb("SourceGroup %u в `condition`, не существует в таблице `mail_loot_template`, игнорнирование.", cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_MILLING_LOOT_TEMPLATE:
        {
            ...
            {
                sLog->outErrorDb("SourceGroup %u в `condition`, не существует в таблице `milling_loot_template`, игнорнирование.", cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_PICKPOCKETING_LOOT_TEMPLATE:
        {
            ...
            {
                sLog->outErrorDb("SourceGroup %u в `condition`, не существует в таблице `pickpocketing_loot_template`, игнорнирование.", cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_PROSPECTING_LOOT_TEMPLATE:
        {
            ...
            {
                sLog->outErrorDb("SourceGroup %u в `condition`, не существует в таблице `prospecting_loot_template`, игнорнирование.", cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_REFERENCE_LOOT_TEMPLATE:
        {
            ...
            {
                sLog->outErrorDb("SourceGroup %u в `condition`, не существует в таблице `reference_loot_template`, игнорнирование.", cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_SKINNING_LOOT_TEMPLATE:
        {
            ...
            {
                sLog->outErrorDb("SourceGroup %u в `condition`, не существует в таблице `skinning_loot_template`, игнорнирование.", cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_SPELL_LOOT_TEMPLATE:
        {
            ...
            {
                sLog->outErrorDb("SourceGroup %u в `condition`, не существует в таблице `spell_loot_template`, игнорнирование.", cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET:
        {
            ...
            if (!spellInfo)
            {
                sLog->outErrorDb("SourceEntry %u в `condition`, не существует в таблице `spell.dbc`, игнорнирование.", cond->SourceEntry);
                return false;
            }

            if ((cond->SourceGroup > MAX_EFFECT_MASK) || !cond->SourceGroup)
            {
                sLog->outErrorDb("SourceEntry %u in `condition` table, has incorrect SourceGroup %u (spell effectMask) set , игнорнирование.", cond->SourceEntry, cond->SourceGroup);
                return false;
            }
            ...
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                ...
                sLog->outErrorDb("SourceEntry %u SourceGroup %u in `condition` table - spell %u does not have implicit targets of types: _AREA_, _CONE_, _NEARBY_ for effect %u, SourceGroup needs correction, игнорнирование.", cond->SourceEntry, origGroup, cond->SourceEntry, uint32(i));
        ...
        case CONDITION_SOURCE_TYPE_CREATURE_TEMPLATE_VEHICLE:
        {
            ...
            {
                sLog->outErrorDb("SourceEntry %u в `condition`, не существует в таблице `creature_template`, игнорнирование.", cond->SourceEntry);
                return false;
            }
            break;
        }
        ...
        case CONDITION_SOURCE_TYPE_QUEST_ACCEPT:
            ...
            {
                sLog->outErrorDb("CONDITION_SOURCE_TYPE_QUEST_ACCEPT specifies non-existing quest (%u), пропуск", cond->SourceEntry);
                return false;
            }
            break;
        case CONDITION_SOURCE_TYPE_QUEST_SHOW_MARK:
            if (!sObjectMgr->GetQuestTemplate(cond->SourceEntry))
            {
                sLog->outErrorDb("CONDITION_SOURCE_TYPE_QUEST_SHOW_MARK specifies non-existing quest (%u), пропуск", cond->SourceEntry);
                return false;
            }
            break;
        case CONDITION_SOURCE_TYPE_VEHICLE_SPELL:
            ...
            {
                sLog->outErrorDb("SourceEntry %u в `condition`, не существует в таблице `creature_template`, игнорнирование.", cond->SourceGroup);
                return false;
            }
...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
    ...
    {
        sLog->outErrorDb("Invalid ConditionType %u at SourceEntry %u in `condition` table, игнорнирование.", uint32(cond->ConditionType), cond->SourceEntry);
        return false;
    }

    if (cond->ConditionTarget >= cond->GetMaxAvailableConditionTargets())
    {
        sLog->outErrorDb("SourceType %u, SourceEntry %u in `condition` table, has incorrect ConditionTarget set, игнорнирование.", cond->SourceType, cond->SourceEntry);
        return false;
    }
    ...
    switch (cond->ConditionType)
    {
        case CONDITION_AURA:
        {
            ...
            {
                sLog->outErrorDb("Aura condition has non existing spell (Id: %d), пропуск", cond->ConditionValue1);
                return false;
            }

            if (cond->ConditionValue2 > EFFECT_2)
            {
                sLog->outErrorDb("Aura condition has non existing effect index (%u) (must be 0..2), пропуск", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_ITEM:
        {
            ...
            if (!proto)
            {
                sLog->outErrorDb("Item condition has non existing item (%u), пропуск", cond->ConditionValue1);
                return false;
            }

            if (!cond->ConditionValue2)
            {
                sLog->outErrorDb("Item condition has 0 set for item count in value2 (%u), пропуск", cond->ConditionValue2);
                return false;
            }
            break;
        }
        case CONDITION_ITEM_EQUIPPED:
        {
            ...
            if (!proto)
            {
                sLog->outErrorDb("ItemEquipped condition has non existing item (%u), пропуск", cond->ConditionValue1);
                return false;
            }
        ...
        case CONDITION_ZONEID:
        {
            ...
            if (!areaEntry)
            {
                sLog->outErrorDb("ZoneID condition has non existing area (%u), пропуск", cond->ConditionValue1);
                return false;
            }

            if (areaEntry->zone != 0)
            {
                sLog->outErrorDb("ZoneID condition requires to be in area (%u) which is a subzone but zone expected, пропуск", cond->ConditionValue1);
                return false;
            }
        ...
        case CONDITION_REPUTATION_RANK:
        {
            ...
            if (!factionEntry)
            {
                sLog->outErrorDb("Reputation condition has non existing faction (%u), пропуск", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_TEAM:
        {
            ...
            {
                sLog->outErrorDb("Team condition specifies unknown team (%u), пропуск", cond->ConditionValue1);
                return false;
            }
        ...
        case CONDITION_SKILL:
        {
            ...
            if (!pSkill)
            {
                sLog->outErrorDb("Skill condition specifies non-existing skill (%u), пропуск", cond->ConditionValue1);
                return false;
            }

            if (cond->ConditionValue2 < 1 || cond->ConditionValue2 > sWorld->GetConfigMaxSkillValue())
            {
                sLog->outErrorDb("Skill condition specifies invalid skill value (%u), пропуск", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_QUEST_COMPLETE:
        {
            ...
            {
                sLog->outErrorDb("Quest condition specifies non-existing quest (%u), пропуск", cond->ConditionValue1);
                return false;
            }
        ...
        case CONDITION_ACTIVE_EVENT:
        {
            ...
            {
                sLog->outErrorDb("ActiveEvent condition has non existing event id (%u), пропуск", cond->ConditionValue1);
                return false;
            }
        ...
        case CONDITION_ACHIEVEMENT:
        {
            ...
            if (!achievement)
            {
                sLog->outErrorDb("Achivement condition has non existing achivement id (%u), пропуск", cond->ConditionValue1);
                return false;
            }
        ...
        case CONDITION_CLASS:
        {
            ...
            {
                sLog->outErrorDb("Class condition has non existing classmask (%u), пропуск", cond->ConditionValue1 & ~CLASSMASK_ALL_PLAYABLE);
                return false;
            }
        ...
        case CONDITION_RACE:
        {
            ...
            {
                sLog->outErrorDb("Race condition has non existing racemask (%u), пропуск", cond->ConditionValue1 & ~RACEMASK_ALL_PLAYABLE);
                return false;
            }
        ...
        case CONDITION_MAPID:
        {
            ...
            if (!me)
            {
                sLog->outErrorDb("Map condition has non existing map (%u), пропуск", cond->ConditionValue1);
                return false;
            }
        ...
        case CONDITION_SPELL:
        {
            if (!sSpellMgr->GetSpellInfo(cond->ConditionValue1))
            {
                sLog->outErrorDb("Spell condition has non existing spell (Id: %d), пропуск", cond->ConditionValue1);
                return false;
            }
        ...
        case CONDITION_LEVEL:
        {
            ...
            {
                sLog->outErrorDb("Level condition has invalid option (%u), пропуск", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_DRUNKENSTATE:
        {
            ...
            {
                sLog->outErrorDb("DrunkState condition has invalid state (%u), пропуск", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue2)
            {
        ...
        case CONDITION_NEAR_CREATURE:
        {
            ...
            {
                sLog->outErrorDb("NearCreature condition has non existing creature template entry (%u), пропуск", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_NEAR_GAMEOBJECT:
        {
            ...
            {
                sLog->outErrorDb("NearGameObject condition has non existing gameobject template entry (%u), пропуск", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_OBJECT_ENTRY:
        {
            switch (cond->ConditionValue1)
            {
                case TYPEID_UNIT:
                    ...
                    {
                        sLog->outErrorDb("ObjectEntry condition has non existing creature template entry  (%u), пропуск", cond->ConditionValue2);
                        return false;
                    }
                    break;
                case TYPEID_GAMEOBJECT:
                    ...
                    {
                        sLog->outErrorDb("ObjectEntry condition has non existing game object template entry  (%u), пропуск", cond->ConditionValue2);
                        return false;
                    }
                    break;
                case TYPEID_PLAYER:
                case TYPEID_CORPSE:
                    ...
                    break;
                default:
                    sLog->outErrorDb("ObjectEntry condition has wrong typeid set (%u), пропуск", cond->ConditionValue1);
                    return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_TYPE_MASK:
        {
            ...
            {
                sLog->outErrorDb("TypeMask condition has invalid typemask set (%u), пропуск", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue2)
        ...
        case CONDITION_RELATION_TO:
        {
            ...
            {
                sLog->outErrorDb("RelationTo condition has invalid ConditionValue1(ConditionTarget selection) (%u), пропуск", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue1 == cond->ConditionTarget)
            {
                sLog->outErrorDb("RelationTo condition has ConditionValue1(ConditionTarget selection) set to self (%u), пропуск", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue2 >= RELATION_MAX)
            {
                sLog->outErrorDb("RelationTo condition has invalid ConditionValue2(RelationType) (%u), пропуск", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_REACTION_TO:
        {
            if (cond->ConditionValue1 >= cond->GetMaxAvailableConditionTargets())
            {
                sLog->outErrorDb("ReactionTo condition has invalid ConditionValue1(ConditionTarget selection) (%u), пропуск", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue1 == cond->ConditionTarget)
            {
                sLog->outErrorDb("ReactionTo condition has ConditionValue1(ConditionTarget selection) set to self (%u), пропуск", cond->ConditionValue1);
                return false;
            }
            if (!cond->ConditionValue2)
            {
                sLog->outErrorDb("mConditionValue2 condition has invalid ConditionValue2(rankMask) (%u), пропуск", cond->ConditionValue2);
                return false;
            }
            break;
        }
        case CONDITION_DISTANCE_TO:
        {
            if (cond->ConditionValue1 >= cond->GetMaxAvailableConditionTargets())
            {
                sLog->outErrorDb("DistanceTo condition has invalid ConditionValue1(ConditionTarget selection) (%u), пропуск", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue1 == cond->ConditionTarget)
            {
                sLog->outErrorDb("DistanceTo condition has ConditionValue1(ConditionTarget selection) set to self (%u), пропуск", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue3 >= COMP_TYPE_MAX)
            {
                sLog->outErrorDb("DistanceTo condition has invalid ComparisionType (%u), пропуск", cond->ConditionValue3);
                return false;
            }
            break;
        }
        ...
        case CONDITION_HP_VAL:
        {
            if (cond->ConditionValue2 >= COMP_TYPE_MAX)
            {
                sLog->outErrorDb("HpVal condition has invalid ComparisionType (%u), пропуск", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_HP_PCT:
        {
            ...
            {
                sLog->outErrorDb("HpPct condition has too big percent value (%u), пропуск", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue2 >= COMP_TYPE_MAX)
            {
                sLog->outErrorDb("HpPct condition has invalid ComparisionType (%u), пропуск", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_WORLD_STATE:
        {
            ...
            {
                sLog->outErrorDb("World state condition has non existing world state in value1 (%u), пропуск", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_TITLE:
        {
            ...
            if (!titleEntry)
            {
                sLog->outErrorDb("Title condition has non existing title in value1 (%u), пропуск", cond->ConditionValue1);
                return false;
            }
            break;
        }
        case CONDITION_UNUSED_19:
            sLog->outErrorDb("Found ConditionTypeOrReference = CONDITION_UNUSED_19 in `conditions` table - игнорнирование");
            return false;
        case CONDITION_UNUSED_20:
            sLog->outErrorDb("Found ConditionTypeOrReference = CONDITION_UNUSED_20 in `conditions` table - игнорнирование");
            return false;
        case CONDITION_UNUSED_21:
            sLog->outErrorDb("Found ConditionTypeOrReference = CONDITION_UNUSED_21 in `conditions` table - игнорнирование");
            return false;
        case CONDITION_UNUSED_24:
            sLog->outErrorDb("Found ConditionTypeOrReference = CONDITION_UNUSED_24 in `conditions` table - игнорнирование");
            return false;
        default:
            break;
    }
