...
extern int main(int argc, char **argv)
{
    ...
    /*if (_PACKAGENAME_CUSTOM != _PACKAGENAME_ETALON)
    {
        sLog->outError("Invalid config. file : %s", cfg_file);
        sLog->outError("Verify that the file exists and has \'[authserver]\' written in the top of the file!");
        return 1;
    }*/
	...
	sLog->outString("Ядро: Unell Core v34");
	sLog->outString("Создатель ядра: Kargatum");
	sLog->outString("Ревизия: Archive");
    sLog->outString("Нажмите <Ctrl-C> чтобы остановить процесс.\n");
...
bool StartDB()
{
    ...
    if (dbstring.empty())
    {
        sLog->outError("БД не определена");
        return false;
    }
    ...
    {
        sLog->outError("Невозможно соедениться с БД");
        return false;
    }
