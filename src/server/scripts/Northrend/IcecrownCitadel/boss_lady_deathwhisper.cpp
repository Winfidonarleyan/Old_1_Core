        ...
        struct boss_lady_deathwhisperAI : public BossAI
        {
            ...
            void MoveInLineOfSight(Unit* who)
            {
                if (!_introDone && me->IsWithinDistInMap(who, 110.0f))
                {
                    ...
                    events.ScheduleEvent(EVENT_INTRO_3, 19000, 0, PHASE_INTRO);
                    events.ScheduleEvent(EVENT_INTRO_4, 26500, 0, PHASE_INTRO);
                    events.ScheduleEvent(EVENT_INTRO_5, 41500, 0, PHASE_INTRO);
                    events.ScheduleEvent(EVENT_INTRO_6, 48500, 0, PHASE_INTRO);
                    events.ScheduleEvent(EVENT_INTRO_7, 60000, 0, PHASE_INTRO);
                }
            }
            ...
            void DamageTaken(Unit* /*damageDealer*/, uint32& damage)
            {
                ...
                if (events.GetPhaseMask() & PHASE_ONE_MASK && damage > me->GetPower(POWER_MANA))
                {
                    ...
                    me->RemoveAurasDueToSpell(SPELL_MANA_BARRIER);
                    // on normal mode the cultists do not get reanimated anymore
                    if (!IsHeroic())
                    {
