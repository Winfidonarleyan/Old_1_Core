                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_RENDING_THROW:
                            if(Unit* victim = me->getVictim())
                                if (victim && victim->IsWithinDistInMap(me, 50.0f, false)) // Todo: Fix the distance
                                    DoCast(victim, SPELL_RENDING_THROW);
                                else
    ...
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            if (player->GetItemCount(49278, false) == 0)
                player->ADD_GOSSIP_ITEM(0, "Yeah, I'm sure safety is your top priority. Give me a rocket pack.", 631, 1);
            player->SEND_GOSSIP_MENU(player->GetGossipTextId(creature), creature->GetGUID());
            return true;
        }
        ...
        bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
        {
            ...
            if (action == 1)
            {
                ...
                if (curItemCount >= 1)
                {
                    creature->MonsterWhisper("You already have my rocket pack!", player->GetGUIDLow());
                    return false;
                }
                ...
                {
                    creature->MonsterWhisper("You don't have any empty space for my rocket pack!", player->GetGUIDLow());
                    return false;
                }
            }
    ...
    public:
        ...
        struct npc_saurfang_gunshipAI : public ScriptedAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    Transport* orgrimmar = me->GetTransport();
                    if (!orgrimmar)
                        break;
                    switch (eventId)
                    {
    ...
    public:
        ...
        class spell_icc_remove_rocket_pack_SpellScript : public SpellScript
        {
            ...
            void HandleEffect(SpellEffIndex /*effIndex*/)
            {
                ...
                hitPlr->DestroyItemCount(49278, 1, true, true);
            }
