...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (player->GetQuestStatus(12540) == QUEST_STATUS_INCOMPLETE)
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        ...
        //npc_escortAI* pEscortAI = CAST_AI(npc_injured_rainspeaker_oracle::npc_injured_rainspeaker_oracleAI, creature->AI());

        if (action == GOSSIP_ACTION_INFO_DEF+1)
        {
            player->CLOSE_GOSSIP_MENU();
            DoScriptText(SAY_START_IRO, creature);
            player->GroupEventHappens(QUEST_FORTUNATE_MISUNDERSTANDINGS, creature);
        }

        if (action == GOSSIP_ACTION_INFO_DEF+2)
        {
            Creature* Crock = creature->SummonCreature(28325, creature->GetPositionX() + 3.0f, creature->GetPositionY() + 3.0f, creature->GetPositionZ(), 0, TEMPSUMMON_CORPSE_DESPAWN);
            if (Crock)
            {
                Crock->Attack(player, true);
            }
        }
        return true;
    }
