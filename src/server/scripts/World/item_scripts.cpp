...
class item_living_fire : public ItemScript
{
public:
    item_living_fire() : ItemScript("item_living_fire") { }

    bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& /*targets*/)
    {
        if (player->GetQuestStatus(9805) == QUEST_STATUS_INCOMPLETE)
        {
            if (player->FindNearestCreature(18110, 10.0f))
                player->KilledMonsterCredit(18110, 0);

            if (player->FindNearestCreature(18142, 10.0f))
                player->KilledMonsterCredit(18142, 0);

            if (player->FindNearestCreature(18143, 10.0f))
                player->KilledMonsterCredit(18143, 0);

            if (player->FindNearestCreature(18144, 10.0f))
                player->KilledMonsterCredit(18144, 0);
        }
...
class item_quel_delar : public ItemScript
{
public:
    item_quel_delar() : ItemScript("item_quel_delar") { }

    bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& /*targets*/)
    {
        if (player->GetZoneId() == 4075)
            player->AddItem(49871, 1);
        return true;
    }
};

class item_essence_of_ice : public ItemScript
{
public:
    item_essence_of_ice() : ItemScript("item_essence_of_ice") { }

    bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& /*targets*/)
    {
        if (player->FindNearestCreature(30169, 10.0f))
            player->AddItem(42252, 1);
        return true;
    }
};
...
void AddSC_item_scripts()
{
    ...
    new item_living_fire();
    new item_quel_delar();
    new item_essence_of_ice();
}
