...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
		...
		if (sTransmogrification->EnableTransmogInfo)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|tКак Работает Трансмогрификация?", EQUIPMENT_SLOT_END+9, 0);
...
#ifdef PRESETS
        if (sT->GetEnableSets())
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG,"|TInterface/RAIDFRAME/UI-RAIDFRAME-MAINASSIST:30:30:-18:0|tУправление Комплектами", EQUIPMENT_SLOT_END+4, 0);
#endif
        player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tУбрать Все Трансмогрификации", EQUIPMENT_SLOT_END+2, 0, "Убрать трансмогрификацию со всех одетых вещей?", 0, false);
        ...
        case EQUIPMENT_SLOT_END+4: // Presets menu
            {
				...
				if (sTransmogrification->EnableSetInfo)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|tКак Работают Комплекты?", EQUIPMENT_SLOT_END+10, 0);
                ...
                if (sT->presetByName[player->GetGUID()].size() < sT->GetMaxSets())
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|tСохранить Комплект", EQUIPMENT_SLOT_END+8, 0);
        ...
        case EQUIPMENT_SLOT_END+6: // view preset
            {
				...
				player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|tИспользовать Комплект", EQUIPMENT_SLOT_END + 5, action, "Используя этот комплект, вы не сможете его продать либо передать.\nВы уверены что хотите продолжить?\n\n" + sT->presetByName[player->GetGUID()][action], 0, false);
        ...
        case EQUIPMENT_SLOT_END+8: // Save preset
            {
                ...
                if (canSave)
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|tСохранить Комплект", 0, 0, "Введите Название Комплекта", cost*sT->GetSetCostModifier()+sT->GetSetCopperCost(), true);
    ...
    void ShowTransmogItems(Player* player, Creature* creature, uint8 slot) // Only checks bags while can use an item from anywhere in inventory
    {
        ...
        if (oldItem)
        {
            ...
            for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
            {
                ...
                if (sT->GetFakeEntry(oldItem->GetGUID()) == newItem->GetEntry())
                    continue;
                ++limit;
				player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sT->GetItemLink(newItem, session), slot, newItem->GetGUIDLow(), "Используя этот комплект, вы не сможете его продать либо передать.\nВы уверены что хотите продолжить?\n\n" + sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sT->GetItemLink(newItem, session) + ss.str(), price, false);
            }
        ...
        player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tУбрать Трансмогрификацию", EQUIPMENT_SLOT_END+3, slot, "Убрать трансмогрификацию из этого слота?", 0, false);
