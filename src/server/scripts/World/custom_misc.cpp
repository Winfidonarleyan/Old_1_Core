...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        player->ADD_GOSSIP_ITEM_EXTENDED(0, "Я хочу ввести промо код.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1, "Введите промо код.", 0, true);
        player->SEND_GOSSIP_MENU(player->GetGossipTextId(creature), creature->GetGUID());
        return true;
    }
    ...
    bool OnGossipSelectCode(Player* player, Creature* creature, uint32 uiSender, uint32 uiAction, const char* code)
    {
        ...
        if (uiAction == GOSSIP_ACTION_INFO_DEF+1)
        {
            QueryResult res = CharacterDatabase.PQuery("SELECT item FROM promo WHERE code = '%s'", code);
            if (res)
            {
                QueryResult res2 = CharacterDatabase.PQuery("SELECT date FROM promo_data WHERE guid = '%u'", player->GetSession()->GetAccountId());

                if (res2)
                {
                    QueryResult res3 = CharacterDatabase.PQuery("SELECT date FROM promo_data WHERE guid = '%u' AND (UNIX_TIMESTAMP(NOW()) - date) > 86400", player->GetSession()->GetAccountId());
                    if (!res3)
                    {
                        creature->MonsterWhisper("Сегодня вы уже вводили промо код на этом аккаунте.", player->GetGUID());
                        player->CLOSE_GOSSIP_MENU();
                        return false;
                    }
                }
                ...
                CharacterDatabase.PQuery("INSERT INTO mail_external (receiver, subject, message, item, item_count) VALUES ('%u','Подарок','Вручаем вам подарок за введенный промо код.','%u',1)", player->GetGUIDLow(), item);
                CharacterDatabase.PQuery("DELETE FROM promo WHERE code = '%s'", code);
                CharacterDatabase.PQuery("INSERT INTO promo_data VALUES('%u', '%s', UNIX_TIMESTAMP(NOW()), '%s')", player->GetSession()->GetAccountId(), player->GetName(), code);
                creature->MonsterWhisper("Подарок придет через некоторое время вам на почту.", player->GetGUID());
            }
            else
            {
                creature->MonsterWhisper("Неправильный или уже использованный промо код.", player->GetGUID());
            }
            player->CLOSE_GOSSIP_MENU();
            return true;
        }
...
class go_lottery_event : public GameObjectScript
{
    public:
        go_lottery_event() : GameObjectScript("go_lottery_event") { }

        bool OnGossipHello(Player* player, GameObject* go)
        {
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Снять с дерева елочный шарик.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
