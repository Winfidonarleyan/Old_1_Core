...
public:
    ...
    void run(void)
    {
        ...
        while (!World::IsStopped())
        {
            ...
            {
                sLog->outError("World Thread зависает, выбивая сервер!");
                ASSERT(false);
            }
        }
...
int Master::Run()
{
    ...
    sLog->outString("Нажмите <Ctrl-C> чтобы остановить проект.\n");
    ...
    if (!pidfile.empty())
    {
        ...
        if (!pid)
        {
            sLog->outError("Невозможно создать PID файл %s.\n", pidfile.c_str());
            return 1;
        }
    ...
    #ifdef _WIN32
    {
        ...
        if (Prio)
        {
            if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
                sLog->outString("Процессу присвоен высокий приоритет");
            else
                sLog->outError("Невозмоно присвоить высокий приоритет процессу");
            sLog->outString("");
        }
    }
    #endif
    ...
    sLog->outString("Завершение процесса...");
...
bool Master::_StartDB()
{
    ...
    if (dbstring.empty())
    {
        sLog->outError("БД World не указана в конфиге!");
        return false;
    }
    ...
    {
        sLog->outError("Невозможно присоединиться к базе World %s", dbstring.c_str());
        return false;
    }
    ...
    if (dbstring.empty())
    {
        sLog->outError("БД Character не указана в конфиге!");
        return false;
    }
    ...
    {
        sLog->outError("Невозможно присоединиться к базе Character %s", dbstring.c_str());
        return false;
    }
    ...
    if (dbstring.empty())
    {
        sLog->outError("БД Login(auth) не указана в конфиге!");
        return false;
    }
    ...
    {
        sLog->outError("Невозможно присоединиться к базе Login(auth) %s", dbstring.c_str());
        return false;
    }
    ...
    if (!realmID)
    {
        sLog->outError("Реалм Айди не определён в конфиге");
        return false;
    }
    sLog->outString("Запущен реалм под номером %d", realmID);
    ...
    sLog->outString("Используется база данных: %s", sWorld->GetDBVersion());
    return true;
}
